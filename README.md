...
- `/팀 목록` — 팀 트리와 팀별 인원 목록 표시
- `/팀 닉네임적용 [대상]` — 팀/직급 정보를 닉네임에 반영(미지정 시 본인)
- `/팀 직급 대상 직급` — 대상에게 직급 역할을 부여하고 닉네임에 반영(기존 직급 역할 제거)
- `/팀 직급목록` — 닉네임에 반영할 직급 역할 이름 목록 확인
- `/팀 직급목록설정 목록` — 직급 역할 목록 설정(쉼표/공백 구분, 순서=우선순위)
- `/팀 정리` — 사람이 한 명도 없는 팀(하위 포함)을 일괄 삭제
- `/팀 나가기` — 내 팀 소속을 해제(닉네임 접미사 제거, 빈 팀 자동 정리)
- 규칙
  - 모든 팀 위에는 길드별 루트팀이 하나 존재합니다(내부적으로 `__ROOT__`).
  - 팀 이름 자체에는 공백이 없고, 공백으로 상하위 경로를 표현합니다.
  - 본인 변경은 누구나 가능, 타인 변경은 관리자 권한 필요.
  - 닉네임 형식: `기존닉 | [팀경로 ...] [직급]` (최대 32자 이내)
    - 길이 제한 시 가장 하위 팀부터 우선 표기하고, 남는 길이에 따라 상위 팀을 앞쪽에 추가합니다. 직급은 항상 마지막에 표시합니다.
    - 직급 기본 목록: `회장, 사장, 부장, 차장, 과장, 대리, 사수, 부사수, 신입` (서버별 `/팀 직급목록설정`으로 변경 가능)
  - 팀 정리: 팀/하위 팀 전체에 소속 인원이 한 명도 없으면 해당 팀을 자동 삭제합니다. 팀 이동 시 이전 팀부터 위로 올라가며 빈 팀을 즉시 정리하며, `/팀 정리`로 일괄 정리할 수 있습니다.
