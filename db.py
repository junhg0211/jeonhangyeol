...
def init_db():
    with get_conn() as conn:
        ...
        try:
            ...
            pass
        try:
            conn.execute("ALTER TABLE guild_settings ADD COLUMN main_chat_channel_id INTEGER")
        except Exception:
            pass
        try:
            conn.execute("ALTER TABLE guild_settings ADD COLUMN announce_channel_id INTEGER")
        except Exception:
            pass
        # 공지사항과 메시지 카운터
        conn.execute(
...
# Main chat & announcement channels
def set_main_chat_channel(guild_id: int, channel_id: int | None) -> None:
    with get_conn() as conn:
        conn.execute(
            "INSERT INTO guild_settings(guild_id, main_chat_channel_id) VALUES(?, ?)\n             ON CONFLICT(guild_id) DO UPDATE SET main_chat_channel_id=excluded.main_chat_channel_id",
            (guild_id, channel_id),
        )

def get_main_chat_channel(guild_id: int) -> int | None:
    with get_conn() as conn:
        cur = conn.execute("SELECT main_chat_channel_id FROM guild_settings WHERE guild_id=?", (guild_id,))
        row = cur.fetchone()
...
def get_announce_channel(guild_id: int) -> int | None:
    with get_conn() as conn:
        cur = conn.execute("SELECT announce_channel_id FROM guild_settings WHERE guild_id=?", (guild_id,))
        row = cur.fetchone()
        return int(row[0]) if row and row[0] is not None else None
...
def add_announcement(guild_id: int, content: str) -> int:
    ...
    with get_conn() as conn:
        cur = conn.execute(
            "INSERT INTO announcements(guild_id, content, created_ts) VALUES(?, ?, ?)",
            (guild_id, content.strip(), now),
        )
...
def list_announcements(guild_id: int):
    with get_conn() as conn:
        cur = conn.execute(
            "SELECT id, content, active FROM announcements WHERE guild_id=? ORDER BY id ASC",
            (guild_id,),
        )
...
def next_announcement(guild_id: int, index: int) -> str | None:
    with get_conn() as conn:
        cur = conn.execute(
            "SELECT content FROM announcements WHERE guild_id=? AND active=1 ORDER BY id ASC",
            (guild_id,),
        )
...
def incr_message_count(guild_id: int, channel_id: int) -> int:
    with get_conn() as conn:
        cur = conn.execute(
            "SELECT count FROM message_counters WHERE guild_id=? AND channel_id=?",
            (guild_id, channel_id),
        )
        row = cur.fetchone()
        if row is None:
            conn.execute(
                "INSERT INTO message_counters(guild_id, channel_id, count) VALUES(?, ?, 1)",
                (guild_id, channel_id),
            )
            return 1
        count = int(row[0]) + 1
        conn.execute(
            "UPDATE message_counters SET count=? WHERE guild_id=? AND channel_id=?",
...
def _ensure_team_root(guild_id: int) -> int:
    with get_conn() as conn:
        ...
        if row:
            return int(row[0])
        cur = conn.execute(
            "INSERT INTO teams(guild_id, name, parent_id) VALUES(?, ?, NULL)",
            (guild_id, TEAM_ROOT_NAME),
        )
        return int(cur.lastrowid)
...
def _get_or_create_child(guild_id: int, parent_id: int, name: str) -> int:
    ...
    with get_conn() as conn:
        ...
        if row:
            return int(row[0])
        cur = conn.execute(
            "INSERT INTO teams(guild_id, name, parent_id) VALUES(?, ?, ?)",
            (guild_id, name, parent_id),
        )
        return int(cur.lastrowid)
...
def ensure_team_path(guild_id: int, path: str) -> int:
    """Ensure a team path like 'A B C' exists under the guild root and return deepest team id."""
...
def list_team_members(guild_id: int, team_id: int):
    with get_conn() as conn:
        cur = conn.execute(
            "SELECT user_id FROM user_teams WHERE guild_id=? AND team_id=? ORDER BY user_id ASC",
            (guild_id, team_id),
        )
